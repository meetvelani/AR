...
const INITIAL_MAP = [
  ...
  { childID: "Everly_Quinn_Clarisse_Sofa", mtl: INITIAL_MTL },
...
let texture = textureLoader.load('img/pattern_.jpg');
...
obj_loader.load(obj_MODEL_PATH, function (obj) {
...
function selectSwatch(e) {
  ...
  if (color.texture) {
    ...
    txt = new THREE.TextureLoader().load(color.texture);
    ...
    txt = parseInt('0x' + color.color)


  }
  setobjMaterial(object, activeOption, new_mtl, color);
...
function setobjMaterial(parent, type, mtl, color) {
  object.traverse(function (child) {
    // console.log(child)
    // child.material = mtl;
    if (child.name === type) {
    ...
    // if (color.texture){
    //   if ( child.isMesh ) child.material = mtl;
